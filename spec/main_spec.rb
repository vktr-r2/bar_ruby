...
RSpec.describe Bartender do
  ...
  describe '#chat' do
    it 'outputs a random chat topic' do
      expect do
        bartender.chat
      end.to output(/How was your day?|The weather today is crazy, right?|Man, the local sports team stinks!/).to_stdout
    end
  end
  ...
  describe '#make_order' do
    ...
    it 'makes an order for an available drink' do
      expect do
        bartender.make_order('Stella',
                             'wine')
      end.to output(/Here is your order:\n\*Frosty glass of Stella\*\n\*Delicious looking wine\*/).to_stdout
    end
  ...
  describe '#recite_menu' do
    it 'lists menu items and beers on tap' do
      ...
      expect do
        bartender.recite_menu
      end.to output(/We serve:\nbeer\nwhiskey\ncocktail\nwine\nFor beer we have:\nStella\nCanadian/).to_stdout
    end
  end
end
